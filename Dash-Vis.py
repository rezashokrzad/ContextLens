...
df2 = pd.read_excel('SemCor_interactive_df_present.xlsx')
df3 = pd.read_excel('SemCor_interactive_df_human.xlsx')
df4 = pd.read_excel('SemCor_interactive_df_domestic.xlsx')
df5 = pd.read_excel('SemCor_interactive_df_cold.xlsx')
df6 = pd.read_excel('SemCor_interactive_df_high.xlsx')
df7 = pd.read_excel('SemCor_interactive_df_clean.xlsx')
df8 = pd.read_excel('SemCor_interactive_df_dirty.xlsx')

#receive tooltip content
tooltip_columns = ['text', 'idx', 'sense', 'corrected_sense', 'five senses']
tooltip_list = df1[tooltip_columns].values.tolist()

#dash setup
app = dash.Dash()
dict_main = {'df_great': df1, 'df_present': df2, 'df_human': df3, 'df_domestic': df4,
             'df_cold': df5, 'df_high': df6, 'df_clean': df7, 'df_dirty': df8}
...
dict_label = {'semcor_sense': 'semcor_sense', 'corrected_sense': 'corrected_sense',
              'five senses': 'five senses', 'simple_majority_voting': 'simple_majority_voting'}
...
hovertext = "%{customdata[0]}<br>idx=%{customdata[1]}<br>SemCor_Label=%{customdata[2]} \
<br>annotated_cluster=%{customdata[3]}<br>annotated_label=%{customdata[4]}<extra></extra>"
...
scatter_plot = go.Scatter3d(x=df1["PCA_BERT_Dim1"],
                            ...
                            marker_color= df1['semcor_sense'],
                            hovertemplate=hovertext,
                            #name='clean',
                            showlegend=False,
                            opacity=0.6)
fig = go.Figure(data=[scatter_plot], layout=go.Layout(scene = dict(xaxis = dict(title='Dim1', tickfont=dict(size=13)),
...
app.layout = html.Div([
    html.H1('SemCor Adjectives\' Embeddings Visualization', style={'text-align': 'center'}),
    ...
    html.Div([html.Label("Method"), dcc.Dropdown(id='method-dropdown',
                               ...
                               multi=False,
                               searchable=False)], style={'width': '22%', 'display': 'inline-block'}),
    html.Div([html.Label("Label-Sytem"), dcc.Dropdown(id='label-dropdown',
...
if __name__ == '__main__':
    app.run_server(debug=True, use_reloader=False, host="0.0.0.0", port=1005)
